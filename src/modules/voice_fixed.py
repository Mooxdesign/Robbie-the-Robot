...
class VoiceModule(threading.Thread):
    ...
    def __init__(self, 
        ...
        self.use_iterate = (self.platform == 'Windows')  # Use iterate on Windows, runAndWait on Linux
    ...
    def _init_engine(self) -> Optional[pyttsx3.Engine]:
        ...
        try:
            ...
            if self.platform == 'Windows':
                # Windows: prefer Microsoft voices
                preferred_names = [
                ...
                if selected_voice:
                    engine.setProperty('voice', selected_voice.id)
                    logger.info(f"[VoiceModule] Selected English voice: {selected_voice.id}")
                elif voices:
                    engine.setProperty('voice', voices[0].id)
                    logger.info(f"[VoiceModule] No English voice found. Using fallback: {voices[0].id}")
                else:
                    logger.error("[VoiceModule] No voices available for TTS.")
            
            # Connect event handlers only on Windows (espeak doesn't support them well)
            if self.platform == 'Windows':
                engine.connect('started-word', self._on_cancel)
                engine.connect('finished-utterance', self._on_completed)
            
            logger.info(f"[VoiceModule] TTS engine initialized (platform={self.platform}, use_iterate={self.use_iterate})")
            return engine
    ...
    def run(self):
        """Main speech thread loop - uses different strategies for Windows vs Linux"""
        ...
        if self.use_iterate:
            # Windows: Use startLoop/iterate pattern (works well with SAPI5)
            self._run_with_iterate()
        else:
            # Linux: Use runAndWait pattern (works better with espeak)
            self._run_with_runandwait()

    def _run_with_iterate(self):
        """Windows threading pattern using startLoop/iterate"""
        try:
    ...
    def _run_with_runandwait(self):
        """Linux threading pattern using runAndWait"""
        logger.info("[VoiceModule] Using runAndWait pattern for Linux/espeak")
        
        try:
            while self._is_alive.is_set():
                # Wait for speech request
                if self._say.wait(0.1):
                    self._say.clear()
                    
                    # Process all queued text
                    while not self._cancel.is_set() and self._is_alive.is_set():
                        with self._text_lock:
                            if not self._text:
                                break
                            text, blocking = self._text.pop(0)
